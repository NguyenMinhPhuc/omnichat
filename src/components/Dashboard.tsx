    ...
    if (user) {
      const fetchUserData = async () => {
        try {
          ...
          if (userResp) {
            setUserRole(userResp.role);
            setDisplayName(userResp.displayName || "");
            setAvatarUrl(userResp.avatarUrl || null);
          }
        } catch (err) {
        ...
        try {
          const cust = await getJSON(
            `/api/customization/${encodeURIComponent(user.uid)}`
          );
          if (cust && cust.data) {
            setCustomization((prev) => ({ ...prev, ...cust.data }));
            setMessages([
              {
                sender: "ai",
                text:
                  cust.data.greetingMessage || customization.greetingMessage,
              },
            ]);
          }
        } catch (err) {
          console.warn("No customization found", err);
        }

        try {
          const sc = await getJSON(
            `/api/users/${encodeURIComponent(user.uid)}/scenarios`
          );
          setScenario(sc ?? []);
        } catch (err) {
          console.warn("No scenarios found", err);
        }

        try {
          const ks = await getJSON(
            `/api/users/${encodeURIComponent(user.uid)}/knowledge-sources`
          );
          setKnowledgeSources(ks ?? []);
        } catch (err) {
          console.warn("No knowledge sources found", err);
        }
      };
      fetchUserData();
    }
