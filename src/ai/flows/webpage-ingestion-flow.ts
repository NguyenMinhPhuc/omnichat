...
const webpageIngestionFlow = ai.defineFlow(
  ...
  async ({ url, apiKey }) => {
    ...
    try {
      ...
      throw new Error(
        `Failed to fetch URL. Please check the address and your network connection.`
      );
    }
    ...
    if (!response.ok) {
      if (response.status === 403) {
        throw new Error(
          `BLOCKED::Access to the URL was denied (403 Forbidden). The website may have anti-scraping measures. Consider using a dedicated web scraping API service for complex sites.`
        );
      }
      throw new Error(
...
): Promise<WebpageIngestionOutput> {
  try {
    // Step 0: Get API Key for the user from SQL-backed users service
    const user = await usersService.get(input.userId);
    if (!user) {
    ...
    if (!apiKey) {
      throw new Error(
        "API Key is missing for this user. Please configure it in your profile or system-wide."
      );
    }
    ...
    const message =
      error instanceof Error
        ? error.message
        : "An unknown error occurred during webpage ingestion.";
